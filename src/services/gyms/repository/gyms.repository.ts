...
import { checkLike } from '@libs/utils';

@Injectable()
export class GymsRepository extends DddRepository<Gym> {
  entity = Gym;

  async find(condition: { search?: string; searchValue?: string; name?: string }, options?: PaginationOptions) {
    return this.getManager.find(this.entity, {
      where: {
        ...stripUndefined({
          name: condition.name,
          ...checkLike(condition.search, condition.searchValue),
        }),
      },
  ...
  async count(condition: { search?: string; searchValue?: string; name?: string }) {
